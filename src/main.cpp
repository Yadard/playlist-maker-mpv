...
std::string get_series_title() {
  ...
  auto show_title = std::prev(cwd.end(), 2);
  title_season += show_title->string();
  auto season_name = std::prev(cwd.end());
  title_season += ".+?(?:";
  title_season += season_name->string();
  title_season += R"()?.+?)";
  title_season.replace_all(' ', ".+");
  return title_season;
}

std::string get_regex(){
  return get_series_title() + R"((\d+))";
}
...
int main(int argc, char** argv) {
  ...
  Regex::Regex re(get_regex().c_str());
  std::vector<fs::directory_entry> episodes;
  for (auto file : fs::directory_iterator(fs::current_path()))
  {
    if(std::find(formats.begin(), formats.end(), file.path().extension().string()) != formats.end()
                && !file.is_directory())
      episodes.push_back(file);
  }
  std::sort(episodes.begin(), episodes.end(), [&](fs::directory_entry& a, fs::directory_entry& b){
    std::string str_a = a.path().filename().string();
    std::string str_b = b.path().filename().string();
    auto number_a = std::stoi(std::string(re.search(str_a.c_str())[1]));
    auto number_b = std::stoi(std::string(re.search(str_b.c_str())[1]));
